...
function toolResultFromJson(data, { isError = false } = {}) {
  const wantStructured = !isError && (toolOutput !== 'text' || toolOutputSchemaMode !== 'none');
  const wantText = toolOutput === 'text' || toolOutput === 'both' || (toolOutputSchemaMode !== 'none' && toolOutput === 'text');
...
const TOOL_DEFS = [
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        tag_name: { type: 'string', description: 'Tag name for the release (e.g., "v3.0.0")' },
        name: { type: 'string', description: 'Release title' },
        body: { type: 'string', description: 'Release notes in Markdown' },
        target_commitish: { type: 'string', description: 'Branch name or commit SHA to tag' },
        ...
        generate_release_notes: { type: 'boolean', description: 'Auto-generate release notes', default: false },
      },
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        license_template: { type: 'string', description: 'License template identifier' },
      },
      required: ['name'],
      additionalProperties: false,
    }
  },
  {
    ...
    description: 'Fork an existing GitHub repository to the authenticated account or organization.',
    annotations: {
      title: 'Fork Repository',
      readOnlyHint: false,
      destructiveHint: false,
    ...
    inputSchema: {
      type: 'object',
      properties: {
        repo_url: { type: 'string', description: 'Full GitHub repository URL to fork' },
        organization: { type: 'string', description: 'Fork into this organization' },
        name: { type: 'string', description: 'Custom name for forked repository' },
        default_branch_only: { type: 'boolean', description: 'Only fork default branch', default: false },
      },
      required: ['repo_url'],
      additionalProperties: false,
    }
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        team_reviewers: { type: 'array', items: { type: 'string' }, description: 'Team slugs to request' },
      },
      required: ['repo_url', 'pull_number'],
      additionalProperties: false,
    }
  ...
  },
  {
    ...
    title: 'Add Labels',
    description: 'Add one or more labels to an issue or pull request without replacing existing labels.',
    annotations: {
      title: 'Add Labels',
      readOnlyHint: false,
      destructiveHint: false,
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        repo_url: { type: 'string', description: 'Full GitHub repository URL' },
        branch_name: { type: 'string', description: 'Branch name to delete (without refs/heads/ prefix)' },
      },
      required: ['repo_url', 'branch_name'],
      additionalProperties: false,
    }
          ...
          properties: {
            groups: {
              ...
              items: {
                ...
                required: ['id', 'title', 'description', 'loaded', 'tool_count'],
                additionalProperties: false,
              },
            },
          },
...
const ALL_TOOLS_BY_NAME = new Map(TOOL_DEFS_WITH_OUTPUT_SCHEMA.map(t => [t.name, t]));
...
const TOOL_GROUPS = {
  ...
  pulls: {
    ...
    toolNames: [
      ...
      'github_create_pull_request', 'github_update_pull_request', 'github_merge_pull_request', 'github_request_reviewers',
    ],
  },
  branches_commits: {
    title: 'Branches and Commits',
    description: 'List branches/commits, compare refs, and create branches.',
    toolNames: ['github_list_branches', 'github_list_commits', 'github_get_commit', 'github_compare', 'github_create_branch', 'github_delete_branch'],
  },
  releases_users: {
    title: 'Releases and Users',
    description: 'Releases and user/org profile info.',
    toolNames: ['github_list_releases', 'github_create_release', 'github_user_info', 'github_list_repos', 'github_create_repo', 'github_fork_repo'],
  },
  files_write: {
  ...
  try {
    ...
    switch (name) {
      ...
      case 'github_request_reviewers': {
        ...
        if (args.team_reviewers !== undefined) body.team_reviewers = args.team_reviewers;
        result = await fetchWithBody(
      ...
      case 'github_add_labels': {
        ...
        if (!Array.isArray(args.labels) || args.labels.length === 0) {
          throw new Error("Parameter 'labels' is required and must be a non-empty array.");
        }
        const { owner, repo } = parseRepoUrl(args.repo_url);
        result = await fetchWithBody(
          `${API_BASE_URL}/repos/${owner}/${repo}/issues/${args.issue_number}/labels`,
      ...
      case 'github_create_release': {
        ...
        if (args.body !== undefined) body.body = args.body;
        if (args.target_commitish !== undefined) body.target_commitish = args.target_commitish;
        if (args.draft !== undefined) body.draft = args.draft;
      ...
      case 'github_create_repo': {
        ...
        if (args.gitignore_template !== undefined) body.gitignore_template = args.gitignore_template;
        if (args.license_template !== undefined) body.license_template = args.license_template;
        result = await fetchWithBody(`${API_BASE_URL}/user/repos`, 'POST', body);
        break;
      }
      case 'github_fork_repo': {
        ...
        const { owner, repo } = parseRepoUrl(args.repo_url);
        const body = {};
        if (args.organization !== undefined) body.organization = args.organization;
        if (args.name !== undefined) body.name = args.name;
        if (args.default_branch_only !== undefined) body.default_branch_only = args.default_branch_only;
        result = await fetchWithBody(`${API_BASE_URL}/repos/${owner}/${repo}/forks`, 'POST', body);
        break;
      }
